...
import { Youtube, Instagram, Mic, Video, CheckCircle, Loader2, Copy, ShieldCheck, Clock, AlertCircle } from 'lucide-react';
...
export default function CreatorSignup({ setView }) {
  ...
  // Status: 'idle' | 'checking' | 'pending' | 'verified'
  ...
  const handleLinkChange = (roleId, value) => {
    ...
    // Reset status if link changes
    if (verificationStatus[roleId] === 'pending' || verificationStatus[roleId] === 'verified') {
      setVerificationStatus(p => ({...p, [roleId]: 'idle'}));
    }
  };
  ...
  // 2. Submit for Review (Updated to be realistic)
  const submitForReview = (roleId) => {
    setVerificationStatus(prev => ({ ...prev, [roleId]: 'checking' }));

    // Simulate the network request to your backend
    setTimeout(() => {
      // Change status to 'pending' instead of 'verified'
  ...
  const handleSubmit = async (e) => {
    ...
    // Calculate status
    // We only count them as "Verified" if the status is strictly 'verified' (which requires admin now)
    // But for signup, we track 'pending' requests.
    ...
    try {
      ...
      await setDoc(doc(db, 'users', uc.user.uid), {
        ...
        // IMPORTANT: Default to false. Verification is now a request.
        verified: false, 
    ...
    <div className="card">
      ...
      <form onSubmit={handleSubmit}>
        <div className="input-group">
          <input className="form-input" placeholder="Full Name" required value={formData.fullName} onChange={e => setFormData({...formData, fullName: e.target.value})} />
        </div>
        <div className="input-group">
        ...
        <div className="input-group">
          <input className="form-input" type="password" placeholder="Password" required value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
        </div>

        <div className="mb-4">
          <label className="input-label">Select Your Roles:</label>
          <div className="role-grid">
            {roles.map(role => (
              <div key={role.id} onClick={() => toggleRole(role.id)} className={`role-btn ${formData.selectedRoles.includes(role.id) ? 'selected' : ''}`}>
                ...
                {formData.selectedRoles.includes(role.id) && <CheckCircle size={16} style={{ marginLeft: 'auto' }} />}
              </div>
            ))}
          </div>
        </div>

        {/* Dynamic Links & Verification UI */}
        {formData.selectedRoles.length > 0 && (
          <div className="mb-6" style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
             <label className="input-label" style={{marginBottom: '1rem'}}>Profile Links & Verification</label>
                 ...
                 <div key={roleId} className="mb-6 last:mb-0">
                   ...
                   <div className="flex items-center gap-2 mb-2">
                     ...
                     <input 
                        ...
                        placeholder={`Paste your ${role.label} URL here`} 
                        style={{ padding: '0.6rem', fontSize: '0.9rem', borderColor: isPending ? '#eab308' : '' }}
                        required
                        value={formData.socialLinks[roleId] || ''}
                        onChange={(e) => handleLinkChange(roleId, e.target.value)}
                        disabled={isPending} 
                     />
                   ...
                   {formData.socialLinks[roleId] && (
                     <div style={{ marginLeft: '38px', background: 'white', padding: '1rem', borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '0.85rem' }}>
                        ...
                        {!currentCode && !isPending && (
                           <button 
                             type="button"
                             onClick={() => startVerification(roleId)}
                             className="btn-link"
                             style={{ display: 'flex', alignItems: 'center', gap: '6px', fontWeight: '600' }}
                           >
                             <ShieldCheck size={16} /> Verify ownership of this link
                           </button>
                        )}

                        {/* STATE: CODE GENERATED (User needs to paste) */}
                        {currentCode && !isPending && (
                          <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                            <p style={{ marginBottom: '0.5rem', color: '#64748b' }}>
                              1. Paste this code into your <strong>{role.label} Bio</strong>:
                            </p>
                            <div className="flex items-center gap-2 mb-3">
                              <code style={{ background: '#f1f5f9', padding: '4px 8px', borderRadius: '4px', fontWeight: 'bold', color: 'var(--primary)', border: '1px solid #cbd5e1' }}>
                                {currentCode}
                              </code>
                              <button 
                                type="button"
                            ...
                            <button 
                              type="button"
                              onClick={() => submitForReview(roleId)}
                              className="btn"
                              disabled={status === 'checking'}
                              style={{ 
                                background: 'var(--primary)', color: 'white', padding: '0.4rem 1rem', fontSize: '0.8rem',
                                display: 'flex', alignItems: 'center', gap: '6px' 
                              }}
                            >
                              {status === 'checking' ? <Loader2 size={14} className="animate-spin" /> : <ShieldCheck size={14} />}
                              {status === 'checking' ? 'Submitting...' : 'I have pasted it, Submit for Review'}
                            </button>
                          </div>
                        )}
                        ...
                        {isPending && (
                          <div className="flex items-start gap-2 text-amber-700 bg-amber-50 p-3 rounded border border-amber-100">
                             <Clock size={16} className="mt-0.5 shrink-0" color="#b45309" />
                             <div>
                               <p style={{fontWeight: '600', color: '#b45309', margin: 0}}>Verification Pending</p>
                               <p style={{margin: 0, color: '#92400e', fontSize: '0.8rem'}}>
                                 We are checking your profile for the code <strong>{currentCode}</strong>. This usually takes 24-48 hours.
                               </p>
                             </div>
                          </div>
                        )}
