  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', uc.user.uid), {
        uid: uc.user.uid, type: 'brand', companyName: formData.companyName, website: formData.website, email: formData.email, joinedAt: new Date().toISOString()
      });
  ...
  return (
    <div className="card">
      ...
      <form onSubmit={handleSubmit}>
        <div className="input-group"><input className="form-input" placeholder="Company Name" required value={formData.companyName} onChange={e => setFormData({...formData, companyName: e.target.value})} /></div>
        <div className="input-group"><input className="form-input" placeholder="Website" required value={formData.website} onChange={e => setFormData({...formData, website: e.target.value})} /></div>
